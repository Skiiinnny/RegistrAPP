...
export class DbService {
  ...
  getFakeData() {
    this.httpClient
      .get('assets/database.sql', { responseType: 'text' })
      .subscribe((data) => {
        this.sqlPorter
  ...
  iniciarSesion(usuario, clave): Promise<Usuario> {
    return this.storage
      .executeSql(
        'SELECT * FROM usuario WHERE cuenta_usuario = ? AND clave_usuario = ?',
        [usuario, clave]
      )
      .then((res) => {
        return {
  ...
  cerrarSesion(id) {
    return this.storage
      .executeSql('SELECT * FROM usuario WHERE id_usuario = ?', [id])
      .then((res) => {
        return {
          id_usuario: res.rows.item(0).id_usuario,
          cuenta_usuario: res.rows.item(0).cuenta_usuario,
  ...
  //Funcion para consultar nombre de usuario en db
  consultarNombreUsuario(usuario) {
    return this.storage.executeSql(
      'SELECT * FROM usuario WHERE cuenta_usuario = ?',
      [usuario]
    ).then((res)=>{
      return{
      cuenta_usuario: res.rows.item(0).cuenta_usuario}
